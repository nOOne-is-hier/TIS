...
for tc in range(1, T + 1):
    ...
    # BFS 탐색을 위한 큐
    # (row, column, 방향, 나무 제거 횟수, 조작 횟수)
    order = deque([(start[0], start[1], 0, 0, 0)])
    ...
    while order:
        r, c, direction, trees_cut, controls = order.popleft()
        ...
        for dir in range(4):
            ...
            if 0 <= nr < N and 0 <= nc < N:
                ...
                if control_count_next < min_controls[nr][nc][trees_cut_next][dir]:
                    ...
                    order.append((nr, nc, dir, trees_cut_next, control_count_next))
